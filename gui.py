...
class ForensicApp(ctk.CTk):
    ...
    def _update_results_tab(self, results: list, anomalies_map: dict,
        ...
        for meta in results:
            ...
            is_image = meta.get("_тип") == "image"
            icon = "🖼️" if is_image else "📄"

            self.results_text.insert("end", f"{'='*58}\n")
            self.results_text.insert("end", f"  {icon} {fname}\n")
            self.results_text.insert("end", f"{'='*58}\n")
            ...
            if is_image:
                fields = [
                    ...
                    "Серийный номер камеры", "Объектив",
                    "ISO", "Диафрагма", "Выдержка", "Фокусное расстояние", "Вспышка",
                    "Дата создания", "Дата оцифровки", "Дата изменения", "Часовой пояс",
                    "ПО обработки", "Автор (Artist)", "Авторские права",
                    "GPS Координаты", "GPS Высота", "GPS Скорость", "GPS Время",
                    "MD5", "SHA256",
                ]
            else:
                fields = [
